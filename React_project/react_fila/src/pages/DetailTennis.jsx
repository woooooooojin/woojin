...
export default function DetailTennis(props) {
    const {tennis} = props
    const {id} = useParams()
  ...
  return (
    <div>
        <div className="detail_wrap">
            ...
            <img src={tennis[id].image} alt="img" />
            </div>
            <div className="detail_right">
            <div className="detail_tit">{tennis[id].title}</div>
            <div className="detail_desc">{tennis[id].desc}</div>
            <div className="detail_price">{(tennis[id].price).toLocaleString('ko-KR')}원</div>
            <div className="detail_option">

                <div className="opt_wrap">
                <label className='lab01' style={colorChange('S')}>
                <input
                className='opt01'
                name="radio_opt"
                type="radio"
                ...
                <label className='lab02' style={colorChange('M')}>
                    <input
                    className='opt02'
                    name="radio_opt"
                    type="radio"
                ...
                <label className='lab03' style={colorChange('L')}>
                    <input
                    className='opt03'
                    name="radio_opt"
                    type="radio"
            ...
            <Button onClick={()=>{dispatch(addItem({id: tennis[id].id, img: tennis[id].image, title: tennis[id].title, option: selectedOption,price : tennis[id].price ,count: 1}),alert('장바구니에 담겼습니다.'))}}>장바구니</Button>
            <Button onClick={()=>{dispatch(addItem({id: tennis[id].id, img: tennis[id].image, title: tennis[id].title, option: selectedOption,price : tennis[id].price ,count: 1}),alert('장바구니를 확인해주세요.'))}}>구매하기</Button>
