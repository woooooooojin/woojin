  ...
  // };
    ...
    <div>
      <div className="detail_wrap">
        ...
        <img src={shoes[id].image} alt="img" />
        </div>
        <div className="detail_right">
        <div className="detail_tit">{shoes[id].title}</div>
        <div className="detail_desc">{shoes[id].desc}</div>
        <div className="detail_price">{(shoes[id].price).toLocaleString('ko-KR')}원</div>
        <div className="detail_option">

            <div className="opt_wrap">
            <label className='lab01' style={colorChange('230mm')}>
            <input
            className='opt01'
            name="radio_opt"
            type="radio"
            value="230mm"
            checked={selectedOption === '230mm'}
            onChange={handleOptionChange}
            />
            230
            </label>

            <label className='lab02' style={colorChange('250mm')}>
                <input
                className='opt02'
                name="radio_opt"
                type="radio"
                value="250mm"
                checked={selectedOption === '250mm'}
                onChange={handleOptionChange}
                />
                250
            </label>

            <label className='lab03' style={colorChange('260mm')}>
                <input
                className='opt03'
                name="radio_opt"
                type="radio"
                value="260mm"
                checked={selectedOption === '260mm'}
                onChange={handleOptionChange}
                />
                260
            </label>
        ...
        <Button onClick={()=>{dispatch(addItem({id: shoes[id].id, img: shoes[id].image, title: shoes[id].title, option: selectedOption,price : shoes[id].price ,count: 1}),alert('장바구니에 담겼습니다.'))}}>장바구니</Button>
        <Button onClick={()=>{dispatch(addItem({id: shoes[id].id, img: shoes[id].image, title: shoes[id].title, option: selectedOption,price : shoes[id].price ,count: 1}),alert('장바구니를 확인해주세요.'))}}>구매하기</Button>
